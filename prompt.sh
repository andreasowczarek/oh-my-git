...
if [ -n "${BASH_VERSION}" ]; then
    ...
    function custom_build_prompt {
        ...
        if [[ $is_a_git_repo == true ]]; then
            ...
            prompt+=$(enrich_append $has_untracked_files $omg_has_untracked_files_symbol "${green_on_white}")
            prompt+=$(enrich_append $has_modifications $omg_has_modifications_symbol "${green_on_white}")
            prompt+=$(enrich_append $has_deletions $omg_has_deletions_symbol "${green_on_white}")
